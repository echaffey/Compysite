...
def T_y(theta_rad):
    ...
    return np.array([[np.cos(theta_rad),  0,  np.cos(theta_rad+np.pi/2)],
                     [0, 1, 0],
...
def T_z(theta_rad):
    ...
    return np.array([[np.cos(theta_rad), np.cos(np.pi/2-theta_rad), 0],
...
def tensor_to_vec(tensor):
    ...
    return np.array(
        [*np.diag(tensor), tensor[1, 2], tensor[0, 2], tensor[0, 1]])
...
def principal_stress_3D(stress_tensor: np.ndarray) -> Union[np.ndarray, np.ndarray]:
    ...
    _p1_index, _p2_index, _p3_index = _e_val_l.index(
        _p1), _e_val_l.index(_p2), _e_val_l.index(_p3)
    _p1_vec, _p2_vec, _p3_vec = _e_vec[:,
                                       _p1_index], _e_vec[:, _p2_index], _e_vec[:, _p3_index]

    # Assembble two vectors containing the principal stresses and their dirctional vectors
    _p_val, _p_vec = np.array([_p1, _p2, _p3]), np.array(
...
def to_gamma(strain_tensor) -> np.ndarray:
    ...
    _gamma_matrix = _strain_tensor + \
        (_strain_tensor - _strain_tensor*np.eye(3))
...
def to_epsilon(strain_matrix) -> np.ndarray:
    ...
    _epsilon_tensor = _strain_matrix * \
        np.eye(3) + 0.5*(_strain_matrix - _strain_matrix*np.eye(3))
